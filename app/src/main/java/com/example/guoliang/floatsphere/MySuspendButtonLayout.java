...
public class MySuspendButtonLayout extends RelativeLayout implements View.OnTouchListener {
    ...
    private final LayoutParams mainImageParams;
    private MotionEvent mMotionEvent;
    private int a;

    public MySuspendButtonLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
...
//            mMarginY = ta.getDimension(R.styleable.SuspendButtonLayout_marginY, 67);
            mMarginY = 0;
...
//            if (mDistance > mMarginY) mDistance = mMarginY;
            ...
            // 按钮布局
            mainImageParams = new LayoutParams((int) mImageSize, (int) mImageSize);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (suspendedStatus == SUSPEND_BUTTON_CLOSED) {
                    int xinX=Main1Activity.getWmParams().x;
//                    int xinY=Main1Activity.getWmParams().y;
//                    moveAnimSingle(imageMain, startX, startY, xinX, xinY, 0, false);
//                    moveAnimAll(imageViewList, xinX-viewX, xinY - viewY, 0);
                    if (Math.abs(lastX - downX) > 1 && Math.abs(lastY - downY) > 1) {
                        suspendedStatus = SUSPEND_BUTTON_MOVING;
                        if (listener != null) {
                            listener.onButtonStatusChanged(suspendedStatus,imageMain);
                        }
                        suspendedStatus = SUSPEND_BUTTON_CLOSED;
                    }
                }
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (suspendedStatus == SUSPEND_BUTTON_CLOSED) {
                    int xinX=Main1Activity.getWmParams().x;
//                    int xinY=Main1Activity.getWmParams().y;
//                    moveAnimSingle(imageMain, startX, startY, 0, xinY, 0, false);
//                    moveAnimAll(imageViewList, xinX , xinY , 0);
                    if (Math.abs(lastX - downX) > 1 && Math.abs(lastY - downY) > 1) {
                ...
                // 点击事件，打开关闭
                if (Math.abs(lastX - downX) < 1 && Math.abs(lastY - downY) < 1) {
//                    imageMain.layout(0,(int) event.getRawY(),0,0);


                    //添加相应的规则
//                    params.addRule(RelativeLayout.ALIGN_BOTTOM,R.id.ll_status);
                    //设置控件的位置
//                    params.setMargins(0, (int) event.getRawX(), 0, 0);//左上右下
//                    imageMain.setLayoutParams(params);
                    openSuspendButton(event);
                    closeSuspendButton(event);
                }
    ...
    private void openAnim() {
            ...
            moveAnimAll(imageViewList, 0, (ints[1] - getStatusBarHeight())-a, 0);
        a = ints[1] - getStatusBarHeight();
        // 显示子按钮
        for (int i = 0; i < imageViewList.size(); i++) {
        ...
        for (int i = 0; i < mNumber; i++) {
            float x, y;
            if (Main1Activity.suspendedInLeft) {
        ...
        vto.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            public void onGlobalLayout() {
                ...
                moveAnimSingleMain(imageMain, startX, startY, endX, endY, 0, false);
                //noinspection deprecation
                imageMain.getViewTreeObserver().removeOnGlobalLayoutListener(this);
            }
        });
                ...
                if (isAll) {
                    switch (suspendedStatus) {
                        case SUSPEND_BUTTON_CLOSING:
                            suspendedStatus = SUSPEND_BUTTON_CLOSED;
                            if (listener != null) {
                                listener.onButtonStatusChanged(suspendedStatus,imageMain,mMotionEvent);
                            }
                            break;
    ...
    private void moveAnimSingleMain(Object object, float startX, float startY, float endX, float endY,
                                    long duration, final boolean isAll) {
        ObjectAnimator animatorX = ObjectAnimator.ofFloat(object, "translationX", startX, endX);
//        ObjectAnimator animatorY = ObjectAnimator.ofFloat(object, "translationY", startY, endY);
        AnimatorSet set = new AnimatorSet();
     ...
     * Open SuspendButton
     */
    public void openSuspendButton(MotionEvent event) {
        if (suspendedStatus == SUSPEND_BUTTON_CLOSED
                && imageMain.getVisibility() == View.VISIBLE) {
            suspendedStatus = SUSPEND_BUTTON_OPENING;
            if (listener != null) {
                listener.onButtonStatusChanged(suspendedStatus,imageMain,event);
            }
            openAnim();
        }
    }
     ...
     * Close SuspendButton
     */
    public void closeSuspendButton(MotionEvent event) {
    ...
    public interface OnSuspendListener {
         ...
         */
        void onButtonStatusChanged(int status, ImageView imageView);
        void onButtonStatusChanged(int status, ImageView imageView, MotionEvent event);
